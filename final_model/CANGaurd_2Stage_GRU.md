# Presentation Q&A — CANGaurd_2Stage_GRU.py

> Answers are written for a presentation audience — clear, direct, and backed by the actual design decisions in the pipeline.  
> This document mirrors `CANGuard_2Stage_LSTM.md` but describes the **BiGRU + Multi-Head Attention** variant in `CANGaurd_2Stage_GRU.py`. The data pipeline, Stage-2 scoring fusion, GAN probes, threshold tuning, and hybrid evaluation logic are **identical** to the LSTM script; only the recurrent backbones differ.
...
- Reads a **sliding window of 24 consecutive CAN frames**, each described by 14 features: CAN ID, DLC, 8 data bytes, and 6 engineered features (inter-arrival time, CAN ID frequency, byte entropy, byte statistics).
...
**Why BiGRU after CNN?**
After local pattern extraction, a **bidirectional GRU** captures sequence-level context in both directions — how the pattern evolves over the full 24-frame window. GRU has fewer parameters than LSTM (no separate cell state), which can train faster on the Car-Hacking subset while still modeling temporal dependencies.

**Why Multi-Head Attention after the first BiGRU?**
Attention lets the model **re-weight time steps** — e.g. focus on the burst frames in a DoS window rather than treating all 24 steps equally. This matches the CANGuard paper’s philosophy (CNN–BiGRU–**Attention**). The block uses self-attention (`query = key = value = x`), a residual connection, and LayerNormalization for stable training.
...
### Stage 2 Model — Ensemble of 3 Sequence-Aware Autoencoders (BiGRU + Attention)
...
The previous model flattened the (24, 14) window into a 336-D vector and fed it to a plain MLP. This destroyed the temporal structure — the fact that frame `t` and frame `t+1` are correlated. The Conv1D + BiGRU + attention encoder learns **temporal dependencies** natively, so normal traffic reconstructs with lower variance. A tighter Normal score distribution means a cleaner separation from anomalous traffic.

**Why Bidirectional GRU in the encoder?**
A forward-only RNN sees each frame conditioned on what came before. A bidirectional one also sees what comes after. In a 24-frame window, both past and future context help build a better compressed representation of the normal pattern.

**Why attention in the AE encoder and decoder?**
The encoder attention highlights which time steps matter most before global pooling into the latent vector `z`. The decoder attention helps the reconstruction path focus on the hardest-to-rebuild timesteps. Together they improve localized anomaly sensitivity (complements the **max per-timestep MSE** score in the fusion).

**Why GlobalAveragePooling1D after encoder attention?**
The LSTM variant used `Bidirectional(LSTM(64, return_sequences=False))`, which collapsed the sequence in the last LSTM layer. Here we keep `return_sequences=True` through BiGRU + attention, then **pool over time** to get a fixed-size vector before the `Dense(32, tanh)` bottleneck. This preserves attention-weighted temporal information in a single latent embedding per window.
...
Step 2 — Stage 1 training (on X_fit, all classes)
  CNN + BiGRU + Attention classifier, 5 output classes
...
Step 3 — Stage 2 AE ensemble training (on Normal-only rows of X_fit)
  ...
  Train 3 independent Conv1D+BiGRU+Attention AEs with different seeds
...
Step 6 — Evaluation on X_test + GAN test windows
  ...
  Report three metric blocks:
    (a) Stage 1 only — 5-class metrics  →  cangaurd_gru_cm_stage1.png
...
## Q6. What is different in our `CANGaurd_2Stage_GRU.py` from the CANGuard base paper (`2603.25763v1.pdf`)?
...
**Our work:** `CANGaurd_2Stage_GRU.py` — a **two-stage hybrid IDS** that adopts CANGuard’s **CNN–BiGRU–Attention** supervised backbone for Stage 1 and extends it with an unsupervised Stage 2 for **unknown (zero-day) attacks**, which the paper explicitly leaves as future work.
...
> **Our pipeline** = CANGuard-style Stage 1 (CNN–BiGRU–Attention) + a new unsupervised Stage 2 for zero-day detection on traffic Stage 1 calls Normal.
...
- Ours: **two-stage**, Car-Hacking, Stage 1 = **CNN–BiGRU–Attention**; Stage 2 = **AE ensemble (BiGRU + attention) + Mahalanobis + GAN-calibrated threshold**.  
...
*(For `CANGaurd_2Stage_GRU.py` — Car-Hacking Dataset pipeline; same as LSTM variant.)*
...
Our `CANGaurd_2Stage_GRU.py` **implements that backbone more faithfully than the LSTM variant** (BiGRU + attention in Stage 1) and extends it with a Stage-2 anomaly detector to handle unknown/zero-day behavior that closed-set models miss.
...
> Our GRU script mirrors its Stage-1 stack and adds a second unsupervised stage to detect unknown attacks among traffic Stage 1 considers normal.
...
> **Our work is based on CANGuard in problem formulation and Stage-1 architecture — CNN, bidirectional GRU, and attention for known-attack classification — and we extend it with a second stage for zero-day detection, which the paper identifies as future work but does not implement.**

We are **not** claiming we copied CANGuard’s exact dataset, hyperparameters, or results. We are claiming we **inherit its core supervised IDS stack** and **extend** it for open-set / unknown attacks.
...
```text
CANGuard (paper)                    Our CANGaurd_2Stage_GRU.py
─────────────────────────────────────────────────────────────────
...
Temporal model (BiGRU)       →      Stage 1: BiGRU(96) → BiGRU(64)
Attention + FC + softmax     →      Stage 1: Multi-Head Attention + Dense + softmax
...
Future work: AE on normals   →      Implemented: AE ensemble (BiGRU+attn) + Mahalanobis + GAN probes
```

**Stage 1 is the closest match to CANGuard among our project scripts.**  
...
#### 2) Same spatio-temporal + attention modeling philosophy

CANGuard’s main insight: CAN attacks are not visible in a single frame — you need **spatial** (per-frame / local), **temporal** (across frames), and **attention** (which timesteps matter).

Our Stage 1 follows that blueprint:

- **Spatial / local:** Conv1D layers extract patterns across features and short time steps.
- **Temporal:** Bidirectional GRU layers model how the window evolves over 24 frames.
- **Attention:** Multi-head self-attention re-weights time steps before the final BiGRU collapse and classification head.

For presentation: *“Stage 1 implements the CANGuard-style CNN–BiGRU–Attention backbone for known attack classification.”*
...
From the paper’s limitations / future work:
...
- We **accept** its supervised CNN–BiGRU–Attention role for known attacks (this script).
- We **extend** it where the paper stops — zero-day / unknown detection, with Stage-2 AEs that also use BiGRU + attention for temporal modeling.
...
```text
                    ...
                    └──────────┬──────────┘
                               │ inherit stack
                               ▼
                    Our Stage 1 (CANGaurd_2Stage_GRU)
                    ┌─────────────────────┐
                    │  CNN + BiGRU + MHA  │
                    │  → softmax (5 cls)  │
                    └──────────┬──────────┘
