# ===============================
# midsem_second_stage_imp.py
# Same hybrid IDS as midsem_model_improvised.py, with an improved Stage 2:
#   - Bottleneck encoder–decoder AE (explicit latent z)
#   - Anomaly score = max( z-score(MSE), z-score(||z||) ) vs train-Normal stats
#   - Threshold chosen on a held-out VAL set by maximizing F1 (Normal vs zero_day)
#     on windows where Stage 1 predicts Normal (GAN probes as positives).
#
# Outputs: midsecond_cm_stage1.png, midsecond_cm_stage2.png, midsecond_cm_final_hybrid.png
# ===============================
...
if not os.path.isdir(data_path):
    ...
    plot_confusion_heatmap(y_s1_true, y_s1_pred, labels5, "Stage 1 (improved pipeline)", "midsecond_cm_stage1.png")
    ...
    if len(y_bin_true) > 0:
        print_metrics_block("Stage 2 (improved: combined score + val F1 threshold)", y_bin_true, y_bin_pred, labels_bin)
        plot_confusion_heatmap(
            y_bin_true,
            y_bin_pred,
            labels_bin,
            "Stage 2 — combined MSE+||z|| score, T from val F1",
            "midsecond_cm_stage2.png",
            figsize=(7, 6),
        )
    ...
    plot_confusion_heatmap(
        ...
        final_pred,
        FINAL_LABELS,
        "Final hybrid (improved Stage 2)",
        "midsecond_cm_final_hybrid.png",
        figsize=(11, 9),
    )

    print("\n--- midsem_second_stage_imp.py complete. Best Stage-2 T={:.6f} (val F1={:.4f}) ---".format(best_T, val_f1))
