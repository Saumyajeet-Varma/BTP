# ===============================
# claude_opus_model.py
# Hybrid IDS (same Stage 1 as midsem_second_stage_imp.py) with a SUBSTANTIALLY
# improved Stage 2 designed to push zero_day PRECISION and ACCURACY up while
# coping with the Normal-heavy class imbalance at inference time.
#
# Stage-2 improvements (vs midsem_second_stage_imp.py):
#   1) Sequence-aware Conv1D + BiLSTM bottleneck autoencoder
#      (operates on (SEQ_LEN, n_feat) windows instead of flattened vectors -
#       captures temporal structure of CAN bus traffic).
#   2) Ensemble of K AEs with different seeds; per-window scores averaged
#      to reduce variance and stabilize the decision boundary.
#   3) Multi-score anomaly detector fused into one robust score:
#         a) Reconstruction MSE (per-window mean)
#         b) Max per-time-step reconstruction MSE  (catches localized anomalies)
#         c) Mahalanobis distance in latent space  (accounts for covariance
#            structure of normal embeddings - far stronger than a plain
#            |z|-z-score).
#      Each score is robust-standardized on train-normal (median / MAD), then
#      they are AVERAGED (not max'd) for a smoother, more discriminative score.
#   4) Threshold tuning is PRECISION-PRIORITIZED:
#         - Build a diverse "synthetic anomaly" set: GAN samples + uniform
#           noise samples + feature-shuffled normal samples (different OOD
#           regimes) - all filtered to those Stage 1 predicts as Normal.
#         - Grid search threshold T to maximize F_beta with beta=0.5 (gives
#           more weight to precision than recall) subject to a soft precision
#           floor PRECISION_FLOOR; fall back to F0.5 if no T meets the floor.
#   5) The hybrid rule and Stage 1 are unchanged so improvements are
#      apples-to-apples comparable to the midsem variant.
#
# Outputs:
...
def build_stage1_model(seq_len, n_features, num_classes=5):
    """Same Stage 1 as midsem_second_stage_imp.py (Stage 1 already works well)."""
...
def build_seq_autoencoder(seq_len, n_features, latent_dim=AE_LATENT_DIM, seed=0):
    """Sequence-aware Conv1D + BiLSTM bottleneck autoencoder.

    Encoder:  (T, F) -> Conv1D x2 -> BiLSTM -> Dense(latent)
...
if not os.path.isdir(data_path):
    ...
    if len(y_bin_true) > 0:
        print_metrics_block(
            "Stage 2 (claude_opus: seq-AE ensemble + Mahalanobis, F0.5 threshold)",
            y_bin_true,
            y_bin_pred,
            labels_bin,
        )
        plot_confusion_heatmap(
            ...
            y_bin_pred,
            labels_bin,
            "Stage 2 — claude_opus (seq-AE ensemble + Mahalanobis)",
            "claude_opus_cm_stage2.png",
            figsize=(7, 6),
        )

    print_metrics_block("Final hybrid (6 classes) - claude_opus_model", y_eval_true_str, final_pred, FINAL_LABELS)
    plot_confusion_heatmap(
        y_eval_true_str,
        final_pred,
        FINAL_LABELS,
        "Final hybrid — claude_opus_model",
        "claude_opus_cm_final_hybrid.png",
        figsize=(11, 9),
    )

    print(
        "\n--- claude_opus_model.py complete."
