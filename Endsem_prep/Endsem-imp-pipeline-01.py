...
if not os.path.isdir(data_path):
    ...
    # 5) Stage 2 — confusion matrix (gated path: Stage1=Normal)
    if n_s2 > 0:
        fig, ax = plt.subplots(figsize=(5.5, 4.5))
        s2_xtick = ["Pred: pass\n(score<=T)", "Pred: anomaly\n(score>T)"]
        s2_ytick = ["True: Normal", "True: Attack"]
        sns.heatmap(
            cm_s2,
            annot=True,
            fmt="d",
            cmap="Oranges",
            ax=ax,
            xticklabels=s2_xtick,
            yticklabels=s2_ytick,
            cbar_kws={"label": "Count"},
        )
        ax.set_title("Stage 2 — Confusion (test | Stage1 predicted Normal only)")
        plt.tight_layout()
        plt.show()
